...
namespace Penta.EeWin.Cp.Base
{
    public class Helpers
    {
        ...
        public static int KerbChangePasswordPack(in KERB_CHANGEPASSWORD_REQUEST kcpr, out IntPtr rgbSerialization, out uint cbSerialization)
        {
            KERB_CHANGEPASSWORD_REQUEST kcprOut = kcpr;

            int cb = Marshal.SizeOf(kcprOut) +
                kcprOut.DomainName.Length +
                kcprOut.AccountName.Length +
                kcprOut.OldPassword.Length +
                kcprOut.NewPassword.Length;

            IntPtr buffer = Marshal.AllocHGlobal(cb);
            IntPtr structBuffer = Marshal.AllocHGlobal(Marshal.SizeOf(kcprOut));

            int pos = Marshal.SizeOf(kcprOut);
            CopyMemory(buffer + pos, kcprOut.DomainName.Buffer, kcprOut.DomainName.Length);
            kcprOut.DomainName.Buffer = (IntPtr)((buffer.ToInt64() + (Int64)pos) - buffer.ToInt64());

            pos += kcprOut.AccountName.Length;
            CopyMemory(buffer + pos, kcprOut.AccountName.Buffer, kcprOut.AccountName.Length);
            kcprOut.AccountName.Buffer = (IntPtr)((buffer.ToInt64() + (Int64)pos) - buffer.ToInt64());

            pos += kcprOut.OldPassword.Length;
            CopyMemory(buffer + pos, kcprOut.OldPassword.Buffer, kcprOut.OldPassword.Length);
            kcprOut.OldPassword.Buffer = (IntPtr)((buffer.ToInt64() + (Int64)pos) - buffer.ToInt64());

            pos += kcprOut.NewPassword.Length;
            CopyMemory(buffer + pos, kcprOut.NewPassword.Buffer, kcprOut.NewPassword.Length);
            kcprOut.NewPassword.Buffer = (IntPtr)((buffer.ToInt64() + (Int64)pos) - buffer.ToInt64());

            Marshal.StructureToPtr(kcprOut, structBuffer, false);
            CopyMemory(buffer, structBuffer, Marshal.SizeOf(kcprOut));
        ...
        public static bool ParseLogonStatus(in string logonStatus)
        {
            if (logonStatus == null)
                return false;

            string[] logonText = { "잠김", "Locked", "로그인됨", "Signed-in" };
            for (int i = 0; i < logonText.Length; i++)
            {
                if (logonStatus.Contains(logonText[i]))
                    return true;
            }
            return false;
        }
